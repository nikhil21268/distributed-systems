...
class RaftNode(raft_pb2_grpc.RaftServiceServicer):
    ...
    def ServeClient(self, request, context):
        print1(f"Node {self.node_id}({self.state}) received client request.")
        data = str()
        leader_id = str()
        success = False
    ...
    def callback_leader_lease_timeout(self):
        ...
        if self.state == 'leader':
            ...
            self.voted_for = None
            self.set_election_timeout()
            print1(f"Node {self.node_id}({self.state}) transitions to follower state for term {self.term}")
            self.leader_lease_remaining_time = None
            self.leader_lease_timeout = None
            self.node_id_with_lease = None
        else:
    ...
    def start_election(self):
        print1(f"Node {self.node_id}({self.state}) timed out for term {self.term}\n")
        if self.state != 'leader':
                ...
                if node_port != str(self.node_port) and self.state != 'leader':
                    ...
                    if response is not None:
                        if response.vote_granted:
                            print1(f"Node {self.node_id}({self.state}) received vote from {node_port}.")
                            self.votes_received += 1
                            self.check_votes()
                        else:
                            print1(f"Node {self.node_id}({self.state}) recieved False vote from {node_port}.")
    ...
    def check_votes(self):
        if self.state == 'candidate':
            if self.votes_received > len(self.nodes) / 2:
                ...
                print1(f"Node {self.node_id}({self.state}) transitions to leader state for term {self.term}")
                self.state = 'leader'
            ...
            if response.term > self.term:
                ...
                if nextIndex < 1:
                    ...
                    try:
                        ...
                        print1(f"Error sending AppendEntries RPC to {self.nodes[i][0] + ':' + self.nodes[i][1]}")
                        return 1
